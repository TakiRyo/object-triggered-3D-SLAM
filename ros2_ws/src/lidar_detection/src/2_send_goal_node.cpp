...
struct ClusterVisitInfo {
  int id; // Unique ID for the cluster
  float cx, cy;
...
public:
  GoalSender() : Node("goal_sender")
  {
    ...
    RCLCPP_INFO(this->get_logger(), "✅ GoalSender (Priority Mode) started.");
  }
...
private:
  ...
  void clusterCallback(const visualization_msgs::msg::MarkerArray::SharedPtr msg)
  {
    ...
    for (const auto &m : msg->markers)
    {
      ...
      cluster.id = clusters_.size(); // Assign ID
      cluster.cx = cx; cluster.cy = cy;

      // Generate 4 Points (Top, Bottom, Left, Right)
      ...
      for (auto &p : raw_points) {
        ...
        // Add offset to push point away from object
        VisitPoint vp;
    ...
    // --- Step 2: Check Status (Did we reach a point?) ---
    // IMPORTANT: Only mark INDIVIDUAL points as visited, not the whole cluster yet.
    for (auto &c : clusters_)
    {
        ...
        for (auto &vp : c.visit_points)
        {
            if (!vp.visited) {
                ...
                if (dist < reach_threshold_) {
                    vp.visited = true;
                    RCLCPP_INFO(this->get_logger(), "📍 Reached a point at cluster ID %d", c.id);
                }
            }
    ...
    // --- Step 3: Priority Selection Logic ---
    VisitPoint *target_point = nullptr;
    
    // LOGIC A: If we are locked on a cluster, stay on it.
    if (active_cluster_id_ != -1) {
        // Find pointer to active cluster
        ClusterVisitInfo* active_c = nullptr;
        for(auto &c : clusters_) { if(c.id == active_cluster_id_) active_c = &c; }

        if (active_c && !active_c->all_visited) {
            // Find nearest point *inside* this cluster
            target_point = getNearestUnvisitedInCluster(active_c);
        } else {
            // Cluster finished or lost, unlock.
            RCLCPP_INFO(this->get_logger(), "🔓 Cluster %d finished. Creating new plan...", active_cluster_id_);
            active_cluster_id_ = -1; 
        }
    }

    // LOGIC B: If no active cluster, find the nearest global cluster
    if (active_cluster_id_ == -1) {
        float min_global_dist = std::numeric_limits<float>::max();
        ClusterVisitInfo* best_cluster = nullptr;

        for (auto &c : clusters_) {
            if (c.all_visited) continue;

            // Check distance to the cluster center (or nearest point)
        ...
        if (best_cluster) {
            active_cluster_id_ = best_cluster->id;
            target_point = getNearestUnvisitedInCluster(best_cluster);
  ...
  void publishGoal(VisitPoint* p) {
      // Avoid spamming the same goal if robot hasn't moved much? 
  ...
  void publishMarkers() {
      ...
      for (auto &c : clusters_) {
        for (auto &vp : c.visit_points) {
          ...
          if (vp.visited) {
            m.color.g = 1.0; m.color.a = 0.8; // Green = Done
          } else if (c.id == active_cluster_id_) {
            m.color.r = 1.0; m.color.g = 1.0; m.color.a = 1.0; // Yellow = Current Target Cluster
          } else {
            m.color.r = 1.0; m.color.a = 0.5; // Red/Dim = Future
          }
  ...
  int active_cluster_id_ = -1; // <--- This is the key to your pipeline
};
